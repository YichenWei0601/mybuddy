...
// Sprite data — all 3 frames per species, {E} = eye placeholder
const ALL_BODIES = {
  duck: [
    ['            ','    __      ','  <({E} )___  ','   (  ._>   ','    `--´    '],
    ['            ','    __      ','  <({E} )___  ','   (  ._>   ','    `--´~   '],
...
function renderSprite(species, eye, hat, frame = 0) {
  const frames = ALL_BODIES[species] || ALL_BODIES.blob
...
const frameCount = ALL_BODIES[species].length
const allFrames = Array.from({ length: frameCount }, (_, i) =>
  renderSprite(species, eye, hat, i)
)
